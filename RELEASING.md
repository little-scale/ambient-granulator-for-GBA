...
6. Tag the build commit and push the tag only after the checks pass:
   ...
   ```sh
   git tag -a v0.13 -m "Ambient Granulator for GBA v0.13"
   git push origin v0.13
   ```

7. Create the GitHub Release from that tag. Upload these files directly from
   `build/release/`:
   - `ambient-granulator-for-gba-v0.13.gba`
   - `ambient-granulator-for-gba-max-load-v0.13.gba`
   - `ambient-granulator-for-gba-patched-test-v0.13.gba`
   - `ambient-granulator-gba-patcher-v0.13.html`
   - `SHA256SUMS.txt`
   - `SAMPLE_LICENSE.md`
